...
class Photoblocks_Public {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script('packery', plugin_dir_url( __FILE__ ) . 'js/packery.pkgd.js', array('jquery'), 0, false);
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/photoblocks.public.js', array( 'jquery', 'anime', 'packery' ), $this->version, false );		
	}
		...
		else
		{
			...
			{	
				...
				{	
					if($has_posts)
					{
						$this->populatePostBlocks($gallery, $blocks);
					}
	
					if(isset($this->values[$atts['id']]["host_name"]) && ! empty($this->values[$atts['id']]["host_name"]))
					{
						foreach($blocks as &$block)
						{
							if($block && ($block->type == "image" || $block->type == "post"))
							{
								$site_host = explode('/', str_replace(array('http://','https://'),'', site_url()));
								$block->image->url = str_replace($site_host[0], $this->values[$atts['id']]["host_name"], $block->image->url);
							}
						}
					}
				}
	...
	public function populatePostBlocks($gallery, &$blocks) 
	{
		if(photob_fs()->can_use_premium_code__premium_only())
		{
			...
			if($gallery["source_post_types"])
				$args['post_type'] = explode(",", $gallery["source_post_types"]);

		
			$relation = "OR";
			if(isset($gallery["source_post_op"]))
				$relation = $gallery["source_post_op"];
			
			$args['tax_query'] = array("relation" => $relation);
			
			if (isset($gallery["source_post_taxonomies"]) && !empty($gallery["source_post_taxonomies"])) 
			{					
				foreach(explode(",", $gallery["source_post_taxonomies"]) as $taxonomy_raw)
				{
			...
			if(count($posts) < count($blocks)) 
			{
				$blocks = array_slice($blocks, 0, count($posts), true);
			}
			else if(count($posts) > count($blocks)) 
			{
				$diff = count($posts) - count($blocks);
				
				$this->appendBlocks($blocks, $diff);
			}
			...
			foreach($posts as $post)
			{
				...
				if($found) {
					...
					if(is_array($thumb)) {
						$block->image->url = $thumb[0];
					}
					$block->click->link = get_permalink($post->ID);
					$block->caption->title->text = $post->post_title;
					$block->caption->description->text = $post->post_excerpt;
					$foundBlocks++;
				}
				unset($post);
				unset($block);
	...
	{	
		...
		if(photob_fs()->is_plan__premium_only("ultimate"))
		{
			if(!empty($this->values[$gallery_id]["host_name"]))
			{
				$timthumb_calculated_path = plugins_url('pbm.php', __FILE__);
				$site_host = explode('/', str_replace(array('http://','https://'),'', site_url()));
				return str_replace($site_host[0], $this->values[$gallery_id]["host_name"], $timthumb_calculated_path);							
			}
		}			
